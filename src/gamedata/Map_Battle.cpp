...
void Map_Battle::set_battlers (vector<Actor_Battler*> battlersTeam1,
		vector<Actor_Battler*> battlersTeam2) {
    //Concatena os vetores dos times
    battlers = battlersTeam1;
    battlers.insert(battlers.end(), battlersTeam2.begin(), battlersTeam2.end());
}

void Map_Battle::set_displayHUD (bool displayHUD){
	this->displayHUD = displayHUD;
}

void Map_Battle::redraw () {
    //Draws map tiles
    for(int i=0; i<height; i++){
        for(int j=0; j<width; j++){
            tileMap[i*width+j].draw(image);
        }
    }
    ...
    for (Actor_Battler* battlersIt : battlers){
        ...
        (battlersIt)->draw(x, y, clip, image);
        ...
        if (!battlersIt->is_acting() && !battlersIt->is_dead() && displayHUD){
            write_text(x, y+80, image, to_string( (int)(battlersIt->get_stamina_percent()) ) +"%", PRETO, 0.5);
            write_text(x, y+100, image, to_string(battlersIt->get_hp()), VERMELHO, 0.5);
        }
    }
}
